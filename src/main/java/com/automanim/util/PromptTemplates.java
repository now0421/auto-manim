...
public final class PromptTemplates {
    ...
    public static final String VISUAL_DESIGN_SYSTEM =
            ...
            + "- The layout field must describe only relative projected placement and spatial"
            + " relationships, not absolute coordinates.\n"
            ...
            + "\n"
            + "Return one JSON object with this shape and field requirements:\n"
            + "{\n"
            + "  \"visual_description\": \"required: what appears on screen and what mathematical relation becomes visible; do not give exact coordinates or animation order\",\n"
            + "  \"scene_mode\": \"optional: `2d` by default, or `3d` only when depth is genuinely needed\",\n"
            + "  \"camera_plan\": \"optional: concise camera orientation or motion, especially for 3d scenes\",\n"
            + "  \"screen_overlay_plan\": \"optional: which text or formulas stay fixed in frame\",\n"
            + "  \"color_scheme\": \"required: main colors, emphasis roles, and styling for labels/highlights/supporting annotations\",\n"
            + "  \"layout\": \"required: relative placement only; describe above/below/left/right/center/corner relationships and preserve semantic geometry such as on-line, above-line, mirrored, attached-to-vertex, or attached-to-segment constraints; never give absolute coordinates\",\n"
            + "  \"animation_description\": \"optional: how the scene evolves so the reasoning becomes visually legible\",\n"
            + "  \"transitions\": \"optional: transition style when useful\",\n"
            + "  \"duration\": \"optional number: approximate duration in seconds\",\n"
            + "  \"color_palette\": [\"optional Manim color names\"]\n"
            + "}\n"
            + "\n"
            ...
            + " and reserve the upper-right safe area for the formula block.\",\n"
    ...
    private static final String STORYBOARD_SCHEMA_GUIDE =
            ...
            + "      \"layout_goal\": \"explicit spatial arrangement derived from the graph node's visual_description and layout, converted into globally consistent coordinates while preserving the intended semantic relationships\",\n"
            ...
            + "          \"placement\": \"exact initial position derived from the graph node's visual_description and layout; coordinates must preserve the intended semantic geometry\",\n"
    ...
    public static final String NARRATIVE_SYSTEM =
            ...
            + "\n"
            + "The user will provide a workflow target together with an ordered step progression"
            + " chain enriched with mathematical and visual context.\n"
            + "\n"
            + "Write a scene-by-scene storyboard that functions as an animation script rather"
            + " than a plain solved answer. The storyboard should form one connected visual"
            + " argument: begin with a clear hook or motivation, introduce foundations before"
            + " advanced content, and make each scene state what appears on screen, what"
            + " changes, and what the student should notice.\n"
            + "\n"
            + "For every scene, explicitly provide:\n"
            + "- the teaching purpose through goal, concise narration, object flow, and ordered actions\n"
            + "- which objects enter, persist, and exit\n"
            + "- stable object ids for all objects\n"
            + "- explicit spatial placement and final coordinates through `layout_goal` and"
            + " `entering_objects.placement`\n"
            + "- a `safe_area_plan` explaining how the layout avoids overflow\n"
            + "\n"
            + "Preserve provided LaTeX formulas exactly when referenced. Turn every visual"
            + " change into explicit staging data, not vague prose.\n"
            + "\n"
            + "Layout and semantic-placement rules:\n"
            + "- Treat the frame as 16:9, with important content kept inside"
            + " x in [-7, 7] and y in [-4, 4].\n"
            + "- Leave about 1 unit of margin from the frame edge.\n"
            + "- Keep simultaneous main visual elements to about 6 to 8.\n"
            + "- Place large formulas near an edge or corner, not over the main geometry.\n"
            + "- If a layout would overflow, split it into multiple scenes or reduce what is shown.\n"
            + "- Use the full ordered graph as a global layout-planning pass so coordinates"
            + " remain consistent across the whole storyboard.\n"
            + "- Keep the diagram/layout as stable as possible from scene to scene; change only"
            + " the necessary layer.\n"
            + "- Read each node's `visual_description` and `layout` as the source of truth for"
            + " what should be visible and how objects should relate spatially before converting"
            + " that intent into absolute placements.\n"
            + "- `layout_goal` and every object `placement` must preserve the node's visual"
            + " semantics, not just screen convenience.\n"
            + "- If the graph specifies relations such as on a line, above or below a line,"
            + " mirrored across a line, centered on a shape, attached to a vertex or segment,"
            + " or positioned relative to another named object, the coordinates must realize"
            + " those relations exactly.\n"
            + "- Choose coordinates with a full-story view so present and future objects,"
            + " including reflected or derived ones, still fit inside the safe area.\n"
            + "- When objects are reflections, mirror images, midpoints, intersections,"
            + " projections, or other derived constructions, their coordinates must be"
            + " geometrically consistent with the referenced objects and lines.\n"
            + "- Use `camera_anchor` and `layout_goal` to prevent layout drift.\n"
            + "\n"
            + "3D scene rules:\n"
            + "- Only use 3D when genuinely needed.\n"
            + "- If a scene uses depth, set `scene_mode` to `3d`, keep `layout_goal` focused on"
            + " projected screen placement, include a concise `camera_plan`, and use"
            + " `screen_overlay_plan` for any text that must remain fixed on screen.\n"
            + "- `camera_plan` should always be explicit for 3D scenes.\n"
            + "\n"
            + "Content selection rules:\n"
            + "- Mathematical enrichment fields such as equations and definitions are optional"
            + " supporting material.\n"
            + "- Use them only when they sharpen the explanation, proof, or visual focus.\n"
            + "- Omit optional math details if they would make the animation redundant,"
            + " overcrowded, or unfocused.\n"
            + "\n"
            + "Problem-solving rules:\n"
            + "- If the target is a math problem, every scene must directly advance the solution.\n"
            + "- Do not collapse the full solution into one dense scene or narration block.\n"
            + "- First establish the problem situation, givens, and what must be found or proved,"
            + " then proceed through the derivation step by step.\n"
            + "- Merge nearby steps when they serve one reasoning move.\n"
            + "- Avoid standalone side scenes for secondary facts, historical remarks, or theorem"
            + " detours unless they are indispensable.\n"
            + "- Keep one stable diagram and evolve it with small changes from scene to scene.\n"
            + "- Auxiliary facts should appear only as brief support unless the whole problem"
            + " centers on them.\n"
            + "- If a formal step is not naturally visual, represent it through a clearer"
            + " intermediate visual beat that preserves the mathematics.\n"
            + "\n"
            + "Preferred scale:\n"
            + "- Prefer 3 to 5 strong scenes for problem-solving workflows unless the lesson"
            + " truly needs more.\n"
    ...
    public static String storyboardCodegenPrompt(String targetConcept, String storyboardJson) {
        return String.format(
                "Target concept: %s\n\n"
                        ...
                        + " safe frame.\n"
                        + "- If any point, marker, endpoint, or tracked object moves, keep its label"
                        + " dynamically attached with `always_redraw(...)` or an updater instead of"
                        + " a one-time `.next_to(...)` placement.\n"
                        + "- If a dependent label is meant to appear only after the motion, build it"
                        + " after the final move or keep it dynamic until reveal time. Do not"
                        + " compute its position early and reveal it later with stale coordinates.\n"
    ...
    public static String codeReviewUserPrompt(String targetConcept,
        ...
        return String.format(
                "Target concept: %s\n"
                        ...
                        + "Focus on continuity between scenes, pacing versus narration, 3D camera"
                        + " readability, fixed-in-frame overlays for explanatory text when needed,"
                        + " and whether semantically constrained visual elements are placed in the"
                        + " correct spatial relationship to the objects they describe.\n"
                        + "Do not act as the final arbiter of literal offscreen or overlap layout"
                        + " defects. A later geometry-based stage will evaluate sampled rendered"
                        + " frames for real offscreen and overlap issues. You may still mention"
                        + " clutter or edge-push risk as advisory evidence when it helps explain"
                        + " continuity or readability concerns, but do not withhold approval for"
                        + " layout risk alone unless it clearly reflects a semantic placement"
                        + " mistake or a storyboard-readability failure.\n"
    ...
    public static final String CODE_GENERATION_SYSTEM =
            ...
            + "- Any label, helper line, brace, arrow, angle marker, or annotation whose position"
            + " depends on a moving object must stay dynamically attached during the motion.\n"
            + "- If a point or marker moves, its label must be implemented with `always_redraw(...)`"
            + " or an updater. Do not place the label once with `.next_to(...)` and then animate"
            + " only the point.\n"
            + "- If such a label appears only after the motion ends, create it after the point"
            + " reaches its final position, or keep it dynamic until it is shown. Do not"
            + " precompute the label position before the move and only `FadeIn` it later.\n"
            + "- If two moving objects define a segment or helper relation, prefer"
            + " `always_redraw(lambda: Line(...))` so the geometry follows automatically.\n"
            + "\n"
    ...
    public static final String CODE_EVALUATION_SYSTEM =
            ...
            + "Your job is to inspect the code and predict whether the generated animation is"
            + " likely to feel visually discontinuous, semantically mis-attached, hard to read"
            + " in 3D, or badly paced against the storyboard narration.\n"
            + "\n"
            ...
            + "- This stage is a pre-render semantic/readability review. A later geometry-based"
            + " scene-evaluation stage will inspect sampled rendered frames for actual offscreen"
            + " placement and actual overlap. Do not duplicate that stage's job.\n"
            ...
            + " transforms as warning signs and advisory evidence, not as final proof of a"
            + " layout failure.\n"
            ...
            + "- Penalize code where a moving point or marker has a static label that was placed"
            + " once with `.next_to(...)` but is not kept attached by `always_redraw(...)` or"
            + " an updater during motion.\n"
            + "- Penalize code where a label or badge is computed relative to an object before"
            + " that object moves, then revealed only afterward, because the stored position"
            + " will usually be stale even if the reveal timing looks correct.\n"
            ...
            + "- `layout_score`: integer 1-10 where higher is better for semantic placement and"
            + " projected readability, not literal sampled overlap/offscreen geometry\n"
            + "- `continuity_score`: integer 1-10 where higher is better\n"
            + "- `pacing_score`: integer 1-10 where higher is better\n"
            + "- `clutter_risk`: integer 1-10 where higher means more advisory readability risk\n"
            + "- `likely_offscreen_risk`: integer 1-10 where higher means more advisory layout risk\n"
            ...
            + "\n"
            + "Score guidance:\n"
            + "- Layout score should drop when element placement is geometrically or semantically"
            + " wrong, when labels likely detach from moving objects, or when 3D projected"
            + " readability is poor.\n"
            ...
            + "- Clutter risk and offscreen risk are advisory heuristics only in this stage."
            + " Do not fail solely because those heuristics look risky if the semantic review"
            + " otherwise passes.\n"
            + "\n"
            + "Be calibrated and pragmatic. Only withhold approval when there are clear,"
            + " viewer-visible semantic/readability risks likely to matter in the rendered output."
