...
public final class ExplorationPrompts {
    ...
    private static final String FOUNDATION_CHECK_SYSTEM =
            "Decide whether the current node is already small, concrete, and self-contained"
                    ...
                    + "- Each node becomes one scene or one major learner-visible beat; foundation nodes must be truly self-explanatory.\n"
                    + "- Judge whether this node can support one clear teaching beat with a concrete visual focus and one main takeaway.\n"
                    + "- A valid teaching beat may be an observation, a failed attempt, a contrast, or a key insight, not only a raw derivation step.\n\n"
                    + "Rules:\n"
    ...
    private static final String PREREQUISITES_SYSTEM =
            "Return the direct prerequisite teaching beats needed before the current node can"
                    ...
                    + "8. Include prerequisite observations or misconceptions when they are needed to make the later insight feel earned.\n\n"
                    + "Output format:\n"
                    + "Return a JSON object with this shape:\n"
                    + "{\n"
                    + "  \"prerequisites\": [\n"
                    + "    {\"step\": \"string, short description of one direct prerequisite teaching beat\", \"reason\": \"string, why this prerequisite is necessary before the current node\"}\n"
                    + "  ]\n"
                    + "}\n\n"
