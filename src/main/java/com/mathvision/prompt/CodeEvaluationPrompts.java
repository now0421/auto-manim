...
public final class CodeEvaluationPrompts {
    ...
    private static final String REVIEW_SYSTEM_MANIM =
            "You are a senior Manim code reviewer.\n"
                    ...
                    + "- `storyboard_contract_compliance` [MANDATORY]: the code preserves the storyboard's hard semantic contracts (`constraints`, `geometry_constraints`, `notes_for_codegen`, `dependency_objects`, `dependency_relation`, `constraint_note`, scene order, and continuity); storyboard objects remain candidate elements rather than a one-for-one rendering checklist, but learner-visible code objects must come from storyboard-declared ids. Fail this check if the code creates an undeclared visible label, caption, formula, marker, helper overlay, explanatory text, or teaching geometry. Do not re-evaluate overall teaching coherence - that was validated upstream.\n"
    ...
    private static final String REVIEW_SYSTEM_GEOGEBRA =
            "You are a senior GeoGebra construction reviewer.\n"
                    ...
                    + "- `storyboard_contract_compliance` [MANDATORY]: the command script preserves the storyboard's hard semantic contracts (`constraints`, `geometry_constraints`, `notes_for_codegen`, `dependency_objects`, `dependency_relation`, `constraint_note`, scene order, and continuity); storyboard objects remain candidate elements rather than a one-for-one rendering checklist, but learner-visible command objects must come from storyboard-declared ids. Fail this check if the script creates an undeclared visible label, caption, formula, marker, helper overlay, explanatory text, or teaching geometry. Do not re-evaluate overall teaching coherence - that was validated upstream.\n"
        ...
        if ("geogebra".equalsIgnoreCase(outputTarget)) {
            return SystemPrompts.buildCurrentRequestSection(String.format(
                    "Figure name: %s\n\n"
                            ...
                            + "Never call a scene placement coordinate such as `x.value` or `y.value` a storyboard hard constraint unless structured constraints, geometry_constraints, notes_for_codegen, or object-registry constraint_note explicitly says that coordinate is fixed.\n"
